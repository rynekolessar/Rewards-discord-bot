...
async def on_reaction_add(reaction, user):
    if(check_leaderboard(reaction.message.id, user.id)):
        if(reaction.emoji == u"\u25B6"):
            ...
            embed = discord.Embed(title = "Leaderboard", color=0xf54076)
            for row in rows:
    ...
    if(reaction.emoji == u"\U0001F44D"):
        ...
        for role in roles:
            if(role.name == "Owner" or role.permissions.administrator):
                permission = True
...
async def points(ctx, command = None, username = None, point = None):
    ...
    if(command == None or point == None or username == None):
        if(command == None and point == None and username == None):
            ...
            await ctx.send("Invalid command, correct format is: \n`!points [add/remove] <username> <points>`")
            return
...
@bot.command(pass_context = True)
async def help(ctx):
    embed = discord.Embed(title = "Help command list", color=0xf54076)
    ...
    embed.add_field(name = "!reset", value = config["reset_help"], inline = False)
